...
Are multiple base currencies used (in orders and configuration)? Run this SQL command: `SELECT value FROM core_config_data WHERE path = 'currency/options/base';` to obtain the current configuration.
...
`SELECT DISTINCT base_currency_code FROM sales_order;` to obtain historical orders data. 
